...
{{--										@else--}}
{{--											<li class="last" id="order_total_price">You Pay<span>${{number_format($total_amount,2)}}</span></li>--}}
{{--                                            <li class="last" id="order_total_price">You Pay<span>${{number_format($cart['amount'],2)}}</span></li>--}}
{{--										@endif--}}
{{--									</ul>--}}
{{--									<div class="button5">--}}
                                ...
                                @foreach(Helper::getAllProductFromCart() as $key=>$cart)
                                    <tr>
                                        ...
                                        <td class="total-amount cart_single_price" data-title="Total">
                                            {{-- @dd(number_format($cart['amount'] * $cart['quantity'], 2)) --}}
                                            <span class="money">${{ number_format($cart['price'] * $cart['quantity'], 2) }}</span>
                                        </td>
                                        <td class="action" data-title="Remove"><a href="{{route('cart-delete',$cart->id)}}"><i class="ti-trash remove-icon"></i></a></td>
                                    </tr>
                                @endforeach
                                    ...
                                    <ul>
                                        ...
                                        @if(session()->has('coupon'))
                                            <li class="coupon_price" data-price="{{Session::get('coupon')['value']}}">
                                                You Save <span>${{ number_format(Session::get('coupon')['value'], 2) }}</span>
                                            </li>
                                        @endif
                                        @php
                                            $total_amount = Helper::totalCartPrice();
    ...
    <script>
        $(document).ready(function(){
            ...
            function updateCartTotals() {
                ...
                $('tr').each(function(){
                    const qtyInput = $(this).find('input.input-number');
                    const qty = parseInt(qtyInput.val());
                    const unitPriceSpan = $(this).find('.unit_price');
                    const unitPrice = parseFloat(unitPriceSpan.data('price'));

                    if (!isNaN(qty) && !isNaN(unitPrice)) {
                        const rowTotal = qty * unitPrice;
                        subtotal += rowTotal;

                        $(this).find('.cart_single_price .money').text(`$${rowTotal.toFixed(2)}`);
                    }
                });
